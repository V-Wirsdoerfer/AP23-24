...
#ax2.plot(lmbd **2, rad_0 / d_0, 'rx', label="Hochrein")
...
ax2.set(
    xlabel="Wellenlänge des Lichts µm",
    ylabel="Winkeländerung"
)

#Ausgleichsrechnung stark dotiert
params, cov = np.polyfit(lmbd_masked **2, abs(rad_2_masked / d_2 - rad_0_masked / d_0), deg=1, cov=True)
def f(lmbd_masked):
    return params[0] * lmbd_masked**2 + params[1]
ax2.plot(lmbd_masked**2, f(lmbd_masked), label="Ausgleichsgerade stark dotiert")

#Ausgleichsrechnung leicht dotiert
params, cov = np.polyfit(lmbd **2, abs(rad_1 / d_1 - rad_0 / d_0), deg=1, cov=True)
def f(lmbd):
    return params[0] * lmbd**2 + params[1]
ax2.plot(lmbd**2, f(lmbd), label="Ausgleichsgerade leicht dotiert")
ax2.legend()
